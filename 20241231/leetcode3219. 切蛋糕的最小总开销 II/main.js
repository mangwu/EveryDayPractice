 ...
 */
var minimumCost = function (m, n, horizontalCut, verticalCut) {
  // 贪心方法：优先选择开销最大的线来切，因为先切会让之后开销大的线被切的次数变少
  // 每次横切都会导致竖切次数增加一次，同理每次竖切都会导致横切次数增加一次
  // => 一条横线，如果被v条竖线穿过，则会被分成v + 1 条横线，但是每次切割成本是一致的
  // => 一条竖线，如果被h条横线穿过，则会被分成h + 1 条竖线，但是每次切割成本是一致的
  ...
  while (horizontalCut.length || verticalCut.length) {
    const hc = horizontalCut[horizontalCut.length - 1] || 0;
    const vc = verticalCut[verticalCut.length - 1] || 0;
    if (hc > vc) {
      // 选择横向切割
      res += (v + 1) * horizontalCut.pop();
      h++;
    } else {
      res += (h + 1) * verticalCut.pop();
      v++;
    }
  }
  return res;
