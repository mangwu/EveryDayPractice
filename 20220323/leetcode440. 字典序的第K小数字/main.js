 ...
 * @LastModifiedDate: 2022-03-24 21:52:28                                      *
 * @ModifiedBy: mangwu                                                         *
 * -----------------------                                                     *
...
var findKthNumber = function (n, k) {
  ...
  // 根据n的大小得出开头是1的个数
  // 如 n < 10 时，1开头的只有1个即1
  // 10 <= n < 20 是，1 开头的有1 + n - 9个 如 15 有 1 10 11 12 13 14 15 共 15 - 8 = 7个
  ...
  for (let i = 1; i <= 9; i++) {
    ...
    let num = Number(i.toString().repeat(digits));
    if (n >= mid) {
      // 有固定的个数
      num = Number(i.toString().repeat(digits));
      if (k > num) {
        // 不在其中
        k = k - num;
      } else {
